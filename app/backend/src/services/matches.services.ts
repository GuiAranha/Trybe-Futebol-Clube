...
export default class MatchsService {
  ...
  public async newMatch(match: IMatches): Promise<IMatches | void> {
    if (match.homeTeam === match.awayTeam) {
      throw new BaseError(400, 'It is not possible to create a match with two equal teams');
    }
    ...
    if (!homeTeam || !awayTeam) {
      throw new BaseError(401, 'There is no team with such id');
    }

    const data = await this.model.create(({ ...match, inProgress: true }));
    return data;
  }
